...
namespace App\Products\CivicBudget\Domain\LegacyImport\Services;
...
use App\Products\CivicBudget\Domain\Communications\Models\CorrespondenceMessage;
use App\Products\CivicBudget\Domain\Communications\Models\MailLog;
use App\Products\CivicBudget\Domain\Communications\Models\ProjectComment;
...
use App\Products\CivicBudget\Domain\Projects\Models\ProjectCoauthor;
use App\Products\CivicBudget\Domain\Projects\Models\ProjectCorrection;
use App\Products\CivicBudget\Domain\Projects\Models\ProjectCostItem;
use App\Products\CivicBudget\Domain\Projects\Models\ProjectStatusLabel;
use App\Products\CivicBudget\Domain\Projects\Models\ProjectVersion;
...
use App\Platform\Users\Models\LegacyAuditLog;
use App\Platform\Users\Models\LegacyPeselRecord;
use App\Platform\Users\Models\LegacyPeselVerificationEntry;
use App\Platform\Users\Models\UserActivationToken;
use App\Products\CivicBudget\Domain\Verification\Enums\BoardType;
use App\Products\CivicBudget\Domain\Verification\Models\AdvancedVerification;
use App\Products\CivicBudget\Domain\Verification\Models\BoardVoteRejection;
use App\Products\CivicBudget\Domain\Verification\Models\ConsultationVerification;
use App\Products\CivicBudget\Domain\Verification\Models\DetailedVerification;
...
use App\Products\CivicBudget\Domain\Verification\Models\LocationVerification;
...
use App\Products\CivicBudget\Domain\Verification\Models\ProjectDepartmentRecommendation;
use App\Products\CivicBudget\Domain\Verification\Models\ProjectDepartmentScope;
use App\Products\CivicBudget\Domain\Verification\Models\ProjectUserAssignment;
use App\Products\CivicBudget\Domain\Verification\Models\VerificationAssignment;
use App\Products\CivicBudget\Domain\Verification\Models\VerificationPressureLog;
...
use App\Products\CivicBudget\Domain\Voting\Models\Vote;
use App\Products\CivicBudget\Domain\Voting\Models\VoteCard;
use App\Products\CivicBudget\Domain\Voting\Models\Voter;
use App\Products\CivicBudget\Domain\Voting\Models\VoterRegistryHash;
    ...
    private function importTaskGroups(array $rows): int
    {
        foreach ($rows as $row) {
            BudgetEdition::query()->updateOrCreate([
                ...
                'name' => 'Edycja SBO '.substr((string) Arr::get($row, 'proposeStart'), 0, 4),
                'year' => (int) substr((string) Arr::get($row, 'proposeStart'), 0, 4),
    ...
    private function importTasks(array $rows): int
    {
        foreach ($rows as $row) {
            ...
            Project::query()->updateOrCreate([
                ...
                'plot' => Arr::get($row, 'plot'),
                'lat' => Arr::get($row, 'lat'),
                'lng' => Arr::get($row, 'lng'),
                'map_lng_lat' => Arr::get($row, 'mapLngLat'),
                'map_data' => $this->decodeLegacyJson(Arr::get($row, 'mapData'), 'tasks.mapData', $this->legacyId($row)),
                'description' => Arr::get($row, 'description'),
                'goal' => Arr::get($row, 'goal'),
                'argumentation' => Arr::get($row, 'argumentation'),
                'availability' => Arr::get($row, 'availability'),
                'recipients' => Arr::get($row, 'recipients'),
                'free_of_charge' => Arr::get($row, 'freeOfCharge'),
                'short_description' => Arr::get($row, 'shortDescription'),
                'additional_cost' => Arr::get($row, 'additionalCost'),
                'local' => Arr::get($row, 'local'),
                'contact_with' => Arr::get($row, 'contactWith'),
                'attachments_anonymized' => (bool) Arr::get($row, 'attachmentsAnonimized', false),
                ...
                'need_correction' => (bool) Arr::get($row, 'needCorrection', false),
                'need_pre_verification' => (bool) Arr::get($row, 'needPreVerification', false),
                'small' => (bool) Arr::get($row, 'small', false),
                'is_picked' => (bool) Arr::get($row, 'isPicked', false),
                'is_hidden' => (bool) Arr::get($row, 'isHidden', false),
                'authors' => $this->decodeLegacyJson(Arr::get($row, 'authors'), 'tasks.authors', $this->legacyId($row)),
                'consent_to_change' => (bool) Arr::get($row, 'consentToChange', false),
                'submitted_at' => $this->nullableLegacyDate(Arr::get($row, 'submitted')),
            ]);
        }
