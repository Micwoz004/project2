...
namespace App\Products\EcoServices\Domain\Schedule\Actions;
...
class ImportCollectionScheduleCsvAction
{
    ...
    {
        Log::info('eco_services.schedule_import.start', [
            'path' => basename($path),
        ...
        if (! is_readable($path)) {
            Log::warning('eco_services.schedule_import.rejected_unreadable_file', [
                'path' => basename($path),
            ]);
        ...
        return DB::transaction(function () use ($path, $scheduleName, $replaceExisting): array {
            ...
            if ($replaceExisting) {
                CollectionScheduleDate::query()
                    ->where('eco_collection_schedule_id', $schedule->id)
                    ->delete();
            }

            $stats = $this->importRows($path, $schedule);

            Log::info('eco_services.schedule_import.success', [
    ...
    private function importRows(string $path, CollectionSchedule $schedule): array
    {
        ...
        foreach ($file as $lineNumber => $row) {
            ...
            if (! $zone instanceof EcoZone || ! $fraction instanceof WasteFraction || blank($payload['collection_date'] ?? null)) {
                $skipped++;
                Log::warning('eco_services.schedule_import.row_skipped', [
                    'schedule_id' => $schedule->id,
                    'line' => $lineNumber + 1,
                ]);
     ...
     */
    private function resolveZone(array $payload): ?EcoZone
    {
        if (filled($payload['zone_id'] ?? null)) {
            return EcoZone::query()->whereKey($payload['zone_id'])->first();
        }
