...
class GeminiWasteRecognitionClient implements WasteRecognitionClient
{
    ...
    {
        $apiKey = config('eco_uslugi.gemini.api_key');
        $model = config('eco_uslugi.gemini.model');

        Log::info('eco_uslugi.waste.recognition.start', [
            'files_count' => count($files),
            'model' => $model,
        ]);

        if (! is_string($apiKey) || $apiKey === '') {
            Log::warning('eco_uslugi.waste.recognition.rejected_missing_api_key');
        ...
        try {
            ...
            Log::info('eco_uslugi.waste.recognition.success', [
                'files_count' => count($results),
            ]);

            return $results;
        } catch (ConnectionException $exception) {
            Log::error('eco_uslugi.waste.recognition.failed', [
    ...
    private function recognizeSingleFile(UploadedFile $file, string $apiKey, string $model): array
    {
        ...
        $response = Http::timeout((int) config('eco_uslugi.gemini.timeout', 20))
