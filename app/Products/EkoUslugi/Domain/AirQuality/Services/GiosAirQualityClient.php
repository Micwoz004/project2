...
class GiosAirQualityClient
{
    ...
    {
        $baseUrl = rtrim((string) config('eko_uslugi.gios.base_url'), '/');

        Log::info('eko_uslugi.gios.stations.start');

        if ($baseUrl === '') {
            Log::warning('eko_uslugi.gios.stations.rejected_missing_base_url');

            return [];
        }

        $payload = Http::timeout((int) config('eko_uslugi.gios.timeout', 12))
            ->get($baseUrl.'/pjp-api/rest/station/findAll')
            ->throw()
            ->json();

        Log::info('eko_uslugi.gios.stations.success', [
