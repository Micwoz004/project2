...
namespace App\Products\EkoUslugi\Http\Controllers\Api\Mobile;

use App\Http\Controllers\Controller;
use App\Products\EkoUslugi\Domain\Address\Models\ResidentAddress;
use App\Products\EkoUslugi\Domain\Address\Services\AddressZoneMatcher;
...
use App\Products\EkoUslugi\Domain\Waste\Services\WasteRecognitionService;
use App\Products\EkoUslugi\Domain\Waste\Services\WasteSearchService;
use App\Products\EkoUslugi\Http\Controllers\EkoUslugiPayload;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Log;

class MobileEkoUslugiController extends Controller
{
    public function overview(EkoUslugiPayload $payload, AirQualitySnapshotService $airQuality): JsonResponse
    {
        Log::info('eko_uslugi.mobile.overview.start');
        ...
        Log::info('eko_uslugi.mobile.overview.success');

        return response()->json($response);
    }

    public function fractions(EkoUslugiPayload $payload): JsonResponse
    {
    ...
    public function searchWaste(Request $request, WasteSearchService $searchService, EkoUslugiPayload $payload): JsonResponse
    {
    ...
    public function pszok(EkoUslugiPayload $payload): JsonResponse
    {
        return response()->json([
    ...
    public function pszokPoint(PszokPoint $point, EkoUslugiPayload $payload): JsonResponse
    {
    ...
    public function news(EkoUslugiPayload $payload): JsonResponse
    {
        return response()->json([
    ...
    public function newsPost(NewsPost $post, EkoUslugiPayload $payload): JsonResponse
    {
    ...
    public function airQuality(EkoUslugiPayload $payload, AirQualitySnapshotService $airQuality): JsonResponse
    {
        return response()->json([
    ...
    public function addresses(Request $request, EkoUslugiPayload $payload): JsonResponse
    {
        return response()->json([
    ...
    public function storeAddress(Request $request, AddressZoneMatcher $matcher, EkoUslugiPayload $payload): JsonResponse
    {
        $validated = $request->validate([
        ...
        $address->forceFill(['eko_zone_id' => $zone?->id])->save();
    ...
    public function upcoming(Request $request, UpcomingCollectionService $upcomingCollectionService, EkoUslugiPayload $payload): JsonResponse
    {
        ...
        if (! $address instanceof ResidentAddress) {
            Log::warning('eko_uslugi.mobile.upcoming.rejected_missing_active_address', [
                'user_id' => $request->user()->id,
            ]);
