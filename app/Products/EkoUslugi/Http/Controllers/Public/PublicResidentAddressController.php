...
class PublicResidentAddressController extends Controller
{
    public function store(Request $request, AddressZoneMatcher $matcher): RedirectResponse
    {
        ...
        Log::info('eko_uslugi.resident_address.store.start', [
        ...
        Log::info('eko_uslugi.resident_address.store.success', [
            'user_id' => $request->user()->id,
            'resident_address_id' => $address->id,
            'eko_zone_id' => $zone?->id,
        ]);

        return redirect()->route('eko-uslugi.addresses')->with('status', 'Adres został zapisany.');
    }
    ...
    public function activate(Request $request, ResidentAddress $address): RedirectResponse
    {
        ...
        Log::info('eko_uslugi.resident_address.activate.start', [
            'user_id' => $request->user()->id,
            'resident_address_id' => $address->id,
        ]);
        ...
        Log::info('eko_uslugi.resident_address.activate.success', [
            'user_id' => $request->user()->id,
            'resident_address_id' => $address->id,
        ]);

        return redirect()->route('eko-uslugi.addresses')->with('status', 'Adres aktywny został zmieniony.');
    }
    ...
    public function destroy(Request $request, ResidentAddress $address): RedirectResponse
    {
        ...
        Log::info('eko_uslugi.resident_address.delete.start', [
            'user_id' => $request->user()->id,
            'resident_address_id' => $address->id,
        ]);
        ...
        Log::info('eko_uslugi.resident_address.delete.success', [
            'user_id' => $request->user()->id,
            'resident_address_id' => $address->id,
        ]);

        return redirect()->route('eko-uslugi.addresses')->with('status', 'Adres został usunięty.');
    }
