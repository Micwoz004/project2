<?php

use App\Console\Commands\ImportLegacyFixtureCommand;
use App\Http\Middleware\AuthenticateMobileToken;
use App\Platform\Http\Middleware\EnsureProductEnabled;
use App\Platform\Http\Middleware\ResolveCurrentClient;
use Illuminate\Foundation\Application;
use Illuminate\Foundation\Configuration\Exceptions;
use Illuminate\Foundation\Configuration\Middleware;

return Application::configure(basePath: dirname(__DIR__))
    ->withCommands([
        ImportLegacyFixtureCommand::class,
    ])
    ->withRouting(
        web: __DIR__.'/../routes/web.php',
        api: __DIR__.'/../routes/api.php',
        commands: __DIR__.'/../routes/console.php',
        health: '/up',
    )
    ->withMiddleware(function (Middleware $middleware): void {
        $middleware->appendToGroup('web', ResolveCurrentClient::class);
        $middleware->appendToGroup('api', ResolveCurrentClient::class);
        $middleware->alias([
            'auth.mobile' => AuthenticateMobileToken::class,
            'product.enabled' => EnsureProductEnabled::class,
        ]);
    })
    ->withExceptions(function (Exceptions $exceptions): void {
        //
    })->create();
