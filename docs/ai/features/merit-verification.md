...
- `ProjectResource` w Filament udostępnia przydzielanie jednostek do typów `MeritInitial`, `MeritFinish` i `Consultation`, wysyłkę kart wstępnych/końcowych/konsultacyjnych oraz cofnięcie wysłanej karty do kopii roboczej. Widoczność akcji wymaga uprawnień weryfikacyjnych i statusów/powrotnego przydziału pasujących do etapu.
