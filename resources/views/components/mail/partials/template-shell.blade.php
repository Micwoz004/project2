@props([
    ...
    'badge' => 'Powiadomienie',
    'badgeColor' => '#e7f6ed',
    'heroKicker' => 'Budżet Obywatelski',
    'heroTitle' => 'Powiadomienie',
    'buttonUrl' => null,
    'buttonLabel' => null,
    'buttonColor' => '#168e45',
    ...
    'notice' => null,
    'noticeColor' => '#168e45',
    'noticeBackground' => '#e7f6ed',
    'supportEmail' => config('mail.from.address'),
    'supportPhone' => config('services.public_support.phone'),
    'cityName' => config('app.name') === 'Laravel' ? 'Miasta' : config('app.name'),
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0">
                                <tr>
                                    ...
                                    <td align="right" style="vertical-align:middle;">
                                        <span style="display:inline-block; padding:8px 12px; border-radius:999px; background:{{ $badgeColor }}; color:#262626; font-size:12px; line-height:1; font-weight:800; text-transform:uppercase; letter-spacing:.08em;">{{ $badge }}</span>
                                    </td>
                                </tr>
                            </table>
                        </td>
                                ...
                                <tr>
                                    <td style="height:138px; background-color:#262626; background-image:linear-gradient(90deg, rgba(22,142,69,.94), rgba(244,198,0,.82));">
                                        <div style="padding:26px 28px; color:#fff;">
                                            <div style="font-size:12px; line-height:1; font-weight:900; text-transform:uppercase; letter-spacing:.12em;">{{ $heroKicker }}</div>
                                            <div style="margin-top:12px; font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif; font-size:36px; line-height:1; font-weight:800;">{{ $heroTitle }}</div>
                                        </div>
                                    </td>
                                </tr>
                            </table>
                    ...
                    @if ($buttonUrl && $buttonLabel)
                        <tr>
                            <td align="center" class="email-pad" style="padding:8px 46px 30px;">
                                <a class="email-button" href="{{ $buttonUrl }}" style="display:inline-block; background:{{ $buttonColor }}; color:{{ $buttonTextColor }}; text-decoration:none; border-radius:999px; padding:16px 28px; font-size:18px; line-height:1; font-weight:800; box-shadow:0 10px 24px rgba(0,0,0,.16);">{{ $buttonLabel }}</a>
                                <p style="margin:18px 0 0; font-size:13px; line-height:1.55; color:#60605d;">Jeśli przycisk nie działa, skopiuj i wklej ten adres w przeglądarce:<br><span style="color:#168e45; word-break:break-all;">{{ $buttonUrl }}</span></p>
                            </td>
                        </tr>
                    @endif
                    @if ($notice)
                        <tr>
                            <td class="email-pad" style="padding:0 46px 42px;">
                                <div style="border-left:5px solid {{ $noticeColor }}; background:{{ $noticeBackground }}; border-radius:0 18px 18px 0; padding:18px 20px; font-size:15px; line-height:1.55; color:#262626;">{{ $notice }}</div>
                            </td>
                        </tr>
                    @endif
                </table>
