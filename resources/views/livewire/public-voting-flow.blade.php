<div>
    <section class="panel">
        @if ($edition)
            <p class="meta">
                <span class="pill">Start {{ $edition->voting_start->format('Y-m-d H:i') }}</span>
                <span class="pill neutral">Koniec {{ $edition->voting_end->format('Y-m-d H:i') }}</span>
            </p>
        @else
    ...
    <div class="grid">
        @forelse ($localProjects as $project)
            <article class="item project-card">
                <p class="meta">
                    <span class="pill">{{ $project->area?->name }}</span>
                    <span class="pill neutral">Nr {{ $project->number_drawn ?? $project->number }}</span>
                </p>
                <h2>{{ $project->title }}</h2>
            </article>
        @empty
