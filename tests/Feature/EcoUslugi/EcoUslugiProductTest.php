...
use App\Products\EcoUslugi\Domain\Notifications\Models\NotificationEvent;
use App\Products\EcoUslugi\Domain\Notifications\Models\NotificationTemplate;
...
it('exposes ekousługi as an enabled platform product', function (): void {
    $response = $this->getJson(route('mobile.products.index'))
        ->assertOk();

    expect(collect($response->json('items'))->pluck('key')->all())
        ->toContain(ProductKey::EcoUslugi->value);
});

it('blocks ekousługi routes when product is disabled for current client', function (): void {
    Client::default()
        ->products()
        ->where('product_key', ProductKey::EcoUslugi->value)
        ->update(['enabled' => false]);

    $this->getJson(route('mobile.eco-uslugi.overview'))
        ->assertNotFound();

    $this->get(route('eco-uslugi.home'))
        ->assertNotFound();
});
...
it('scopes waste search results by current client', function (): void {
    ...
    $otherClient->products()->create([
        'product_key' => ProductKey::EcoUslugi->value,
        'enabled' => true,
        'settings' => [],
    ]);
    ...
    $this->getJson(route('mobile.eco-uslugi.waste.search', ['query' => 'karton']))
        ->assertOk()
        ->assertJsonPath('items.0.instruction', 'Wyrzuć do papieru.')
        ->assertJsonCount(1, 'items');

    $this->withHeader('X-Client-Slug', 'drugi-klient')
        ->getJson(route('mobile.eco-uslugi.waste.search', ['query' => 'karton']))
        ->assertOk()
...
it('matches resident address to a zone and returns upcoming collections', function (): void {
    ...
    $this->withHeader('Authorization', 'Bearer '.$token)
        ->postJson(route('mobile.eco-uslugi.resident.addresses.store'), [
            'label' => 'Dom',
            'locality' => 'Szczecin',
            'street' => 'Jasna',
            'building_number' => '12',
    ...
    $this->withHeader('Authorization', 'Bearer '.$token)
        ->getJson(route('mobile.eco-uslugi.resident.schedules.upcoming'))
        ->assertOk()
...
it('renders the ekousługi resident spa shell', function (): void {
    $this->get(route('eco-uslugi.home'))
        ->assertOk()
        ->assertSee('Ekousługi')
