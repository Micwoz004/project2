...
use App\Products\EkoUslugi\Domain\AirQuality\Models\AirQualityReading;
...
use App\Products\EkoUslugi\Domain\Notifications\Models\NotificationEvent;
...
it('exposes eko usługi as an enabled platform product', function (): void {
    $response = $this->getJson(route('mobile.products.index'))
        ->assertOk();

    expect(collect($response->json('items'))->pluck('key')->all())
        ->toContain(ProductKey::EkoUslugi->value);
});

it('blocks eko usługi routes when product is disabled for current client', function (): void {
    Client::default()
        ->products()
        ->where('product_key', ProductKey::EkoUslugi->value)
        ->update(['enabled' => false]);

    $this->getJson(route('mobile.eko-uslugi.overview'))
        ->assertNotFound();

    $this->get(route('eko-uslugi.home'))
        ->assertNotFound();
});
...
it('scopes waste search results by current client', function (): void {
    ...
    WasteItem::query()->create([
        'eko_waste_fraction_id' => $defaultFraction->id,
        'name' => 'Karton',
    ...
    $otherClient->products()->create([
        'product_key' => ProductKey::EkoUslugi->value,
        'enabled' => true,
        'settings' => [],
    ]);
    ...
    WasteItem::query()->create([
        'eko_waste_fraction_id' => $otherFraction->id,
        'name' => 'Karton lokalny',
    ...
    $this->getJson(route('mobile.eko-uslugi.waste.search', ['query' => 'karton']))
        ->assertOk()
        ->assertJsonPath('items.0.instruction', 'Wyrzuć do papieru.')
        ->assertJsonCount(1, 'items');

    $this->withHeader('X-Client-Slug', 'drugi-klient')
        ->getJson(route('mobile.eko-uslugi.waste.search', ['query' => 'karton']))
        ->assertOk()
...
it('matches resident address to a zone and returns upcoming collections', function (): void {
    ...
    $this->withHeader('Authorization', 'Bearer '.$token)
        ->postJson(route('mobile.eko-uslugi.resident.addresses.store'), [
            'label' => 'Dom',
            'locality' => 'Szczecin',
            'street' => 'Jasna',
            'building_number' => '12',
    ...
    $this->withHeader('Authorization', 'Bearer '.$token)
        ->getJson(route('mobile.eko-uslugi.resident.schedules.upcoming'))
        ->assertOk()
...
it('imports collection schedule dates from csv', function (): void {
    ...
    $path = tempnam(sys_get_temp_dir(), 'eko-schedule-');
    ...
    expect($stats['imported'])->toBe(1)
        ...
        ->and(CollectionScheduleDate::query()
            ->where('eko_zone_id', $zone->id)
            ->where('eko_waste_fraction_id', $fraction->id)
...
it('renders the eko usługi resident spa shell', function (): void {
    $this->get(route('eko-uslugi.home'))
        ->assertOk()
        ->assertSee('Eko usługi')
